...
const EditorAboutProject = ({
  document,
  setDocument,
}: EditorAboutProjectProps) => {
  const technologiesList = TECHNOLOGIES.map((t) => ({
    value: t.name,
    label: t.badgeName,
    iconImg: (
      <img
        src={`https://simpleicons.org/icons/${t.logoName}.svg`}
        alt={t.badgeName}
  ...
  return (
    <>
      <WysiwygEditor
        ...
        onChange={(e) =>
          setDocument({ ...document, about: sanitize(e.target.value) })
        }
      />
      <div className="grid w-full max-w-sm items-center gap-2">
        ...
        <Input
          ...
          onChange={(e) =>
            setDocument({ ...document, projectScreenshot: e.target.value })
          }
        />
      </div>
      <div className="grid w-full items-center gap-2">
        ...
        <MultiSelect
          ...
          onValueChange={(value) =>
            setDocument({
              ...document,
              technologies: value as TechnologyName[],
            })
          }
